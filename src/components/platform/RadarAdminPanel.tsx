...
import { defaultSiteOverrides, normalizeSiteOverrides, SITE_OVERRIDES_KEY, type SiteOverrides } from "@/lib/site-overrides";
...
function load(): SiteOverrides { if (typeof window === "undefined") return initial; try { const raw = window.localStorage.getItem(SITE_OVERRIDES_KEY); return raw ? normalizeSiteOverrides({ ...initial, ...JSON.parse(raw) }) : initial; } catch { return initial; } }
...
export function RadarAdminPanel() {
  ...
  useEffect(() => {
    fetch("/api/studio", { cache: "no-store" }).then((response) => response.json()).then((payload) => {
      if (payload?.settings) { setValues(normalizeSiteOverrides(payload.settings)); setShared(Boolean(payload.configured)); }
      if (payload?.content) {
  ...
  const login = async () => { const response = await fetch("/api/studio", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password }) }); setAuthenticated(response.ok); setPassword(""); setMessage(response.ok ? "Admin access enabled." : "Incorrect admin password."); };
  const logout = async () => { await fetch("/api/studio", { method: "DELETE" }); setAuthenticated(false); setMessage("Admin access ended."); };
  const publish = async () => {
    ...
    setMessage(shared ? "Saved to the shared site database." : "Saved to this browser. Database is not configured yet.");
    setSaved(true);
  };
  const reset = async () => {
    if (shared && !authenticated) { setMessage("Log in before resetting shared changes."); return; }
    const next = { ...initial };
    if (shared) {
      const response = await fetch("/api/studio", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ settings: next }) });
      if (!response.ok) { setMessage("Shared reset failed; no changes were made."); return; }
    }
    window.localStorage.setItem(SITE_OVERRIDES_KEY, JSON.stringify(next));
    setValues(next); window.dispatchEvent(new Event("radar-overrides-updated"));
    setMessage(shared ? "Shared settings reset to the site defaults." : "Browser settings reset to the site defaults."); setSaved(true);
  };
  ...
  const updateSocial = (index: number, key: "label" | "href" | "enabled", value: string | boolean) => update("socialLinks", values.socialLinks.map((link, itemIndex) => itemIndex === index ? { ...link, [key]: value } : link));
  const addSocial = () => update("socialLinks", [...values.socialLinks, { label: "New profile", href: "https://", enabled: true }]);
  const removeSocial = (index: number) => update("socialLinks", values.socialLinks.filter((_, itemIndex) => itemIndex !== index));
  ...
  return <main className="min-h-screen bg-paper px-4 pb-24 pt-24 text-ink md:px-8"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-14"><h2 className="display border-b-2 border-ink pb-3 text-3xl">Player & destinations</h2><div className="mt-5 grid gap-5 md:grid-cols-2"><label className={`${labelClass} flex items-center gap-3`}><input type="checkbox" checked={values.playlistEnabled} onChange={(e) => update("playlistEnabled", e.target.checked)} className="h-5 w-5 accent-[var(--flare)]" />Show persistent playlist player</label><label className={labelClass}>YouTube playlist ID<input value={values.playlistId} onChange={(e) => update("playlistId", e.target.value)} className={fieldClass} /></label><label className={labelClass}>Player tooltip / track label<input value={values.playlistLabel} onChange={(e) => update("playlistLabel", e.target.value)} className={fieldClass} /></label><label className={labelClass}>RADARMe URL<input value={values.radarMeUrl} onChange={(e) => update("radarMeUrl", e.target.value)} className={fieldClass} /></label></div></section>
    ...
    <section className="mt-14"><div className="flex items-end justify-between gap-4 border-b-2 border-ink pb-3"><div><h2 className="display text-3xl">Social profiles</h2><p className="mt-2 max-w-2xl font-mono text-xs uppercase leading-relaxed text-muted-foreground">These links power the monochrome footer icon pack. Disable a profile to hide it without deleting its URL.</p></div><button onClick={addSocial} className="brut-border px-4 py-3 font-mono text-xs font-bold uppercase tracking-widest">Add profile</button></div><div className="mt-5 grid gap-4">{values.socialLinks.map((link, index) => <div key={`${link.label}-${index}`} className="grid gap-3 brut-border p-4 md:grid-cols-[auto_1fr_2fr_auto_auto] md:items-center"><input type="checkbox" checked={link.enabled} onChange={(e) => updateSocial(index, "enabled", e.target.checked)} className="h-5 w-5 accent-[var(--flare)]" aria-label={`Enable ${link.label}`} /><input value={link.label} onChange={(e) => updateSocial(index, "label", e.target.value)} className={fieldClass} aria-label={`Label for social profile ${index + 1}`} /><input value={link.href} onChange={(e) => updateSocial(index, "href", e.target.value)} className={fieldClass} aria-label={`URL for social profile ${index + 1}`} /><span className={labelClass}>{link.enabled ? "Visible" : "Hidden"}</span><button onClick={() => removeSocial(index)} className="font-mono text-xs font-bold uppercase tracking-widest text-flare">Remove</button></div>)}</div></section>
    <section className="mt-14"><div className="flex flex-wrap items-end justify-between gap-4 border-b-2 border-ink pb-3"><div><h2 className="display text-3xl">CMS archive & pages</h2><p className="mt-2 max-w-2xl font-mono text-xs uppercase leading-relaxed text-muted-foreground">Create and edit pages, posts, drafts, active records, featured images, tags, categories, and archive entries. Use <strong>status: draft</strong> to keep a record out of public archives; use <strong>published</strong> to make it live after Save / publish.</p></div><select value={contentType} onChange={(e) => selectContentType(e.target.value as keyof ContentCollections)} className="brut-border bg-transparent p-3 font-mono text-xs font-bold uppercase tracking-widest">{(["pages", "articles", "magazine", "artists", "releases", "radarProjects", "events", "categories", "archives"] as (keyof ContentCollections)[]).map((type) => <option key={type} value={type}>{type}</option>)}</select></div><textarea value={contentDraft} onChange={(e) => { setContentDraft(e.target.value); setSaved(false); }} className="mt-5 min-h-[32rem] w-full brut-border bg-transparent p-4 font-mono text-xs leading-relaxed outline-none focus:ring-2 focus:ring-flare" spellCheck={false} aria-label={`${contentType} CMS JSON`} /></section>
