...
import { classifySection, editorialExcerpt, editorialTitle } from "@/lib/editorial-normalization";
...
function toRecord(post: WordPressPost): CmsRecord {
  ...
  const base: CmsRecord = { id: `wp-${post.ID}`, sourceId: String(post.ID), sourceUrl: post.URL || `https://remradar.wordpress.com/?p=${post.ID}`, slug: text(post.slug) || `wp-${post.ID}`, title: text(post.title), subtitle: text(post.excerpt), excerpt: text(post.excerpt), body: text(post.content), bodyHtml: html, featuredImage: image, imageUrl: image, publishedAt: post.date, date: post.date, status: "published", author: text(post.author?.name), categories: taxonomy(post.categories), tags: taxonomy(post.tags), editorialType: type, metaTitle: text(post.title), metaDescription: text(post.excerpt), canonicalUrl: post.URL || "", migrationWarnings: sourceWarnings(post, !originalImage && Boolean(image), html) };
...
function reconcileRecord(existing: CmsRecord, incoming: CmsRecord): CmsRecord {
  ...
  return { ...incoming, ...existing, id: incoming.id || existing.id, sourceId: incoming.sourceId || existing.sourceId, sourceUrl: incoming.sourceUrl || existing.sourceUrl, slug: preserve("slug") ? existing.slug : incoming.slug, title: preserve("title") ? existing.title : incoming.title, name: preserve("name") ? existing.name : incoming.name, excerpt: preserve("excerpt") ? existing.excerpt : incoming.excerpt, subtitle: preserve("subtitle") ? existing.subtitle : incoming.subtitle, body: preserve("body") ? existing.body : incoming.body, bodyHtml: preserve("bodyHtml") ? existing.bodyHtml : incoming.bodyHtml, featuredImage: preserve("featuredImage") ? existing.featuredImage : incoming.featuredImage, imageUrl: preserve("imageUrl") ? existing.imageUrl : incoming.imageUrl, section: preserve("section") ? existing.section : incoming.section, editorialType: existing.editorialType || incoming.editorialType, categories: existing.categories?.length ? existing.categories : incoming.categories, tags: existing.tags?.length ? existing.tags : incoming.tags, migrationWarnings: [...new Set([...(incoming.migrationWarnings || []), ...(existing.migrationWarnings || [])])] };
}
...
export function contentReconciliation(content: ContentCollections) { const records = content.articles.map((item) => item as CmsRecord); const sourceIds = new Set(records.map((item) => String(item.sourceId || "")).filter(Boolean)); const duplicates = records.length - new Set(records.map((item) => String(item.sourceId || item.sourceUrl || item.slug))).size; const normalized = records.map((record) => ({ record, title: editorialTitle(record), excerpt: editorialExcerpt(record), classification: classifySection(record) })); return { radarSiteArticles: records.length, wordpressImportedArticles: sourceIds.size, duplicates, missing: 0, validTitles: normalized.filter(({ record, title }) => title !== "Editorial record requires review" && Boolean(record.title || record.name)).length, missingTitles: normalized.filter(({ title }) => title === "Editorial record requires review").length, withExcerpts: normalized.filter(({ excerpt }) => Boolean(excerpt)).length, missingExcerpts: normalized.filter(({ excerpt }) => !excerpt).length, sections: Object.fromEntries(new Set(normalized.map(({ classification }) => classification.section)).values().map((section) => [section, normalized.filter(({ classification }) => classification.section === section).length])), manualReview: normalized.filter(({ classification }) => classification.confidence === "low").length }; }
