...
export async function generateMetadata({ params }: { params: Promise<{ story: string }> }) { const { story } = await params; const { magazine } = await readPublishedContent(); const item = findBySlug(magazine, story); return item ? { title: item.metaTitle || item.title, description: item.metaDescription || item.subtitle } : { title: "Magazine" }; }
